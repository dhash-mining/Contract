...
// T+1: accounting
// T+2: revenue
contract StakeDHM is Pausable, Ownable {
    ...
    function _stake_(
        ...
        if (
            (cached_epochs[beneficial_].epoch == 0 &&
                cached_epochs[beneficial_].amount == 0) &&
            stakes[beneficial_] > 0
        ) {
            // user's current stake will be counted as a whole for next EPOCH
            // this has to be called before adding user stakes
            _cache_next_epoch(beneficial_, current_epoch, stakes[beneficial_]);
        }
    ...
    function balance_to_claim(address u)
        public
        view
        whenNotPaused
        returns (uint256)
    ...
    function report_timestamp_reward(
        ...
        if (!rewards_each_epoch[epc].valid) {
            ...
            // TODO: check if new data is valid,
